...
def fuzz_post_id():
    ...
    print(f"[*] Pokreće se fuzzing GET {TARGET_URL}{ENDPOINT_BASE}:postId sa {len(payloads)} payloadova.")
    
    for payload in payloads:
        try:
            response = requests.get(
                f"{TARGET_URL}{ENDPOINT_BASE}{payload}",
                headers=headers,
                verify=False,
                timeout=5
            )
            ...
            elif response.status_code not in (404, 403, 401, 200, 201):
            ...
            if "UnicodeEncodeError" in str(e):
                 ...
                 log_fuzz_result(message, details=f"Originalni Payload: {payload}. Raw Auth Token: {RAW_AUTH_TOKEN}")
            pass
